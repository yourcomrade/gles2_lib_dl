...
int main(){
    ...
    printf("Size of s1: %d, Size of import shader: %d\n", sizeof(s1), sizeof(import_shader));
    char *res = (char*)malloc(sizeof(s1) + sizeof(import_shader));
    if(res == NULL){
        perror("Cannot allocate memory for new string!");
        abort();
    }
    strcpy(res, import_shader);
    //strcpy(res, s1);
    strcat(res, s1);
    printf("String res: %s\n", res);
    free(res);
